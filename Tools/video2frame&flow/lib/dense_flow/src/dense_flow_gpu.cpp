...
void calcDenseFlowGPU(string file_name, int bound, int type, int step, int dev_id,
    ...
    while(true){
        ...
        if (!initialized)
        {
            ...
            imencode(".png", prev_image, str_img);
            output_img.push_back(str_img);
            //////////////////////over(2/4 [extract_flow_gpu.cpp;common.cpp;common.h])
        ...
        else 
        {
            ...
            imencode(".png", capture_image, str_img);
...
void calcDenseFlowGPU2(string file_name, int bound, int type, int step, int dev_id,
                      vector<Mat> & output_x,
                      vector<Mat> & output_y,
                      vector<vector<uchar> >& output_img,
                      int new_width, int new_height){
    ...
    bool initialized = false;
    int cnt = 0;
    //namedWindow("MyWindow", CV_WINDOW_AUTOSIZE); ////DEBUG1(1/3)
    while(true){
        ...
        else 
        {
            ...
            //imshow("MyWindow",flow_x);      ////DEBUG1(2/3)
            //waitKey(0);
            ...
            output_x.push_back(flow_x.clone());
            output_y.push_back(flow_y.clone());
            output_img.push_back(str_img);

            //imshow("MyWindow",output_x.back());      ////DEBUG1(3/3)
            //waitKey(0);
