...
void new_calcDenseFlowGPU(string file_name, int bound, int type, int step, int dev_id,
    ...
    std::ifstream fin("/home/alan/files/tmp.txt");  
    string img_name;
    int len;
    ...
    while(true){
        ...
        if (!initialized)
        {
            ...
            getline(fin,img_name);
            //len = strlen(img_name);
            //img_name[len-1]='\0';
            //std::cout << img_name << std::endl; 
            capture_frame=imread(img_name);
