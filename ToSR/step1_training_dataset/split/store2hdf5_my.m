function [curr_dat_sz, curr_lab_sz] = store2hdf5_my(filename, data, labels, create, startloc, chunksz)  
...
%   assert(lab_dims(end)==num_samples, 'Number of samples should be matched between data and labels');
  ...
  if create
    ...
    h5create(filename, '/data', [dat_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [dat_dims(1:end-1) chunksz*7]); % width, height, channels, number 
