...
class SRRaGANModel(BaseModel):
    ...
    def feed_data(self, data, need_HR=True):
        ...
        if need_HR:  # train or val
            HHHRRR=data['HR']
            self.var_H = HHHRRR[:,:3,:,:].to(self.device)
            if HHHRRR.size(1) >3:
                flow_type=3 #3:flownet 4:tvl1
                self.weight_map = HHHRRR[:,flow_type:flow_type+1,:,:].to(self.device)

            input_ref = data['ref'] if 'ref' in data else data['HR']
            self.var_ref = input_ref[:,:3,:,:].to(self.device)
    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            if self.cri_pix:  # pixel loss: WMSE
                l_g_pix = self.l_pix_w * self.cri_pix(self.fake_H*self.weight_map, self.var_H*self.weight_map)
                l_g_total += l_g_pix
